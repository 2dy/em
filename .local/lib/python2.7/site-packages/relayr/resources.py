...
class User(object):
    ...
    def register_wunderbar(self):
        ...
        for k, v in res.items():
            if 'modelId' in v:
                item = Device(res[k], client=self.client)
                item.get_info()
            else:
                item = Transmitter(res[k], client=self.client)
                item.get_info()
            yield item
...
class Group(object):
    ...
    def create(self, name):
        """
        Create new device group with given name.
        ...
        if self.id is None:
            res = self.client.api.post_user_device_group(name)
            self.id = res['id']
        return self
...
class Device(object):
    ...
    def get_info(self):
        ...
        for k in res:
            if k == 'modelId':
                self.model = DeviceModel(res[k], client=self.client)
                self.model.get_info()
            else:
                setattr(self, k, res[k])
        return self
    ...
    def send_command(self, command):
        ...
        :param command: the command to be sent (containing three key strings:
            'path', 'command' and 'value', 'path' being optional)
        :type command: dict
        """
    ...
    def send_config(self, config):
        """
        Sends a config package to configure the device.

        :param config: the configuration to be sent (containing three key strings:
            'path', 'command' and 'value', 'path' being optional)
        :type data: dict
        """
        res = self.client.api.post_device_configurations(self.id, config)
        return res

    def get_config(self, path=None, name=None):
        """
        ...
        :type data: dict
        """
        res = self.client.api.get_device_configurations(self.id, path=path, name=name)
        return res
    ...
    @deprecated(msg='Deprecated method. Please use `send_command()` instead.')
    ...
    def received_data(self, data):
        """
        A callback for live data received for this device.

        The default implementation is to append the reading to an instance
        attribute, `self.buffered_readings`. More customized functionality can
        be implemented in a subclass or by using an other callable.

        This is slightly experimental and subject to change!

        :param reading: a callable that will be called for each new device reading
        :type reading: whatever is returned by the transport mechanism (usually
            a dictionary)
        :rtype: self
        """
        obj = json.loads(data)
        if 'deviceId' in obj:
            del obj['deviceId']
        self.buffered_readings.append(obj)

    def get_live_data(self, callback=None, duration=None, transport='mqtt'):
        """
        Get a stream of current data for this device.

        This is slightly experimental and subject to change!

        :param callback: a callable that will be called for each new device reading
        :type callback: callable Python function or method
        :param duration: duration until the callable is no longer called
        :type duration: timedelta or number
        :param transport: transport method to be used (currently only 'mqtt' which is default)
        ...
        if duration:
            assert type(duration) in [datetime.timedelta, float, int] # + long in Py2...
        if transport == 'mqtt':
            callback_payload_only = lambda topic, payload: callback(payload)
            self.stream = MqttStream(callback_payload_only, [self])
            self.stream.start()
            if duration:
                if type(duration) is datetime.timedelta:
                    ...
                    delay = duration
                stop_it = lambda: self.stream.stop()
                threading.Timer(delay, stop_it).start()

    def stop_live_data(self, transport='mqtt'):
        """
    ...
    def get_past_data(self, start=None, end=None, duration=None, meaning=None, sample=None, offset=None, limit=None):
        """
        ...
        be None, else an ``AssertionError`` is raised. The data will be fetched
        using a paging mechanism with up to 10000 data points per page (``limit``).
        The data will not be stored inside this object.
        ...
        :param meaning: a single device meaning (if unspecified all meanings will be reported)
        :type meaning: string
        :param sample: ...
        :type sample: string
        :param offset: offset in the list of returned data points
        :type offset: integer >= 0
        :param limit: max. total number of returned data points
        :type limit: integer >= 0 (max. 10000)
    ...
    @deprecated(msg='Deprecated method, to be superseeded by `get_past_data()`.')
    def get_data(self, start=None, end=None, duration=None, meaning=None, sample=None, offset=None, limit=None):
        return self.get_past_data(start=start, end=end, duration=duration, meaning=meaning, sample=sample, offset=offset, limit=limit)
